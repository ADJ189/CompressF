<script lang="ts">
	...
	const tools = [
		...
		{ href: '/compress/pdf',    icon: '📄', label: 'PDF',     sub: 'Re-encode pages · pdf-lib',        color: '#c92a2a',       bg: 'rgba(224,49,49,.08)' },
	...
	function download(entry: FileEntry) {
		...
		const ext = fmt.includes('webp') ? 'webp' : fmt.includes('jpeg') ? 'jpg'
			: fmt.includes('png') ? 'png' : fmt.includes('avif') ? 'avif'
			: fmt.includes('pdf') ? 'pdf' : fmt.includes('webm') ? 'webm'
			: fmt.includes('mp4') ? 'mp4' : fmt.includes('mp3') ? 'mp3'
			: fmt.includes('aac') ? 'm4a' : fmt.includes('opus') ? 'opus'
			: fmt.includes('flac') ? 'flac' : fmt.includes('wav') ? 'wav'
			: fmt.includes('ogg') ? 'ogg' : fmt.includes('gif') ? 'gif'
			: fmt.includes('svg') ? 'svg' : 'bin';
...
<div class="page">
	...
	<DropZone
		accept="image/*,video/*,audio/*,.pdf,.gif,.svg,.heic,.heif,.flac,.opus,.aiff,.aif,.mkv,.avi"
		{hasFiles}
		onFiles={addFiles}
		icon="📂"
		title="Drop any files here"
		subtitle="Images · PDF · Video · Audio · GIF · SVG"
	/>
	...
	<div class="tips">
		<span class="tip">💡 For advanced video options — codecs, bitrate, FPS — use the <a href="/compress/video">Video Encoder</a>.</span>
		<span class="tip">🔒 Nothing is uploaded. All processing runs locally in your browser.</span>
	</div>
</div>
...
<style>
	...
	@media (max-width: 700px) { .tool-grid { grid-template-columns: 1fr 1fr; } .tc-body span { display: none; } }
